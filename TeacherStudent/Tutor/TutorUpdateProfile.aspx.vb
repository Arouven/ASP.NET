Public Class TutorUpdateProfile
	...
	Private Sub showCourseDetails()
		...
		cmd.CommandText = "select * from TutorTable where TutorId=@TutorId;"
	...
	Private Function UsernameExistInTutorTable(userName As String)
		...
		Dim cmd As New SqlClient.SqlCommand()
		cmd.CommandType = CommandType.Text
		cmd.CommandText = "select count(UserName) from TutorTable where UserName=@UserName;"
		cmd.Connection = con
		'create a parameterized query
		cmd.Parameters.AddWithValue("@UserName", userName)
		'Create DataReader
	...
	Private Function EmailExistInTutorTable(Email As String)
		...
		Dim cmd As New SqlClient.SqlCommand()
		cmd.CommandType = CommandType.Text
		cmd.CommandText = "select count(Email) from TutorTable where Email=@Email;"
		cmd.Connection = con
		'create a parameterized query
		cmd.Parameters.AddWithValue("@Email", Email)
		'Create DataReader
	...
	Protected Sub ButtonRegister_Click(sender As Object, e As EventArgs)
		...
		If (FileUpload1.HasFile) Then
			...
			If (CheckFileType(FileUpload1.FileName)) Then
				...
				If (UsernameExistInTable("StudentTable", TextBoxUsernameReg.Text)) OrElse (UsernameExistInTable("AdminTable", TextBoxUsernameReg.Text)) Then
				...
				ElseIf (EmailExistInTable("StudentTable", TextBoxEmail.Text)) OrElse (EmailExistInTable("AdminTable", TextBoxEmail.Text)) Then
				...
				Else
					...
					cmd1.CommandText = "update TutorTable set 
FirstName=@FirstName, 
LastName=@LastName, 
...
where Tutorid=@Tutorid;"
					...
					cmd1.Parameters.AddWithValue("@Tutorid", TutorId)
					con1.Open()
