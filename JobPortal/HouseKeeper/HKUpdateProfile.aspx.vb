Public Class HKUpdateProfile
	...
	Private Function UsernameExistInHouseOwnerTable(userName As String)
		Dim con As New SqlClient.SqlConnection(_conString)
		Dim cmd As New SqlClient.SqlCommand()
		cmd.CommandType = CommandType.Text
		cmd.CommandText = "select count(UserName) from HouseOwnerTable where UserName=@UserName;"
		cmd.Connection = con
		'create a parameterized query
		cmd.Parameters.AddWithValue("@UserName", userName)
		'Create DataReader
	...
	'Private Function EmailExistInTutorTable(Email As String)
	'	Dim con As New SqlClient.SqlConnection(_conString)
	'	Dim cmd As New SqlClient.SqlCommand()
	'	cmd.CommandType = CommandType.Text
	'	cmd.CommandText = "select count(Email) from HouseOwnerTable where Email=@Email;"
	'	cmd.Connection = con
	'	'create a parameterized query
	'	cmd.Parameters.AddWithValue("@Email", Email)
	'	'Create DataReader
	...
	Protected Sub ButtonRegister_Click(sender As Object, e As EventArgs)
		...
		If (FileUpload1.HasFile) Then
			...
			If (CheckFileType(FileUpload1.FileName)) Then
				...
				If (UsernameExistInTable("HouseKeeperTable", TextBoxUsernameReg.Text)) OrElse (UsernameExistInTable("AdminTable", TextBoxUsernameReg.Text)) Then
					lblStatus.Text = "Username Already Exist, Please Choose Another"
					lblStatus.ForeColor = System.Drawing.Color.Red
					TextBoxUsernameReg.Focus()
				ElseIf (UsernameExistInHouseOwnerTable(TextBoxUsernameReg.Text)) Then
					lblStatus.Text = "Username Already Exist, Please Choose Another"
					lblStatus.ForeColor = System.Drawing.Color.Red
					TextBoxUsernameReg.Focus()
				...
				Else
					...
					cmd1.CommandText = "update HouseKeeperTable set 
FirstName=@FirstName, 
LastName=@LastName, 
...
where HKid=@HKid;"
					...
					cmd1.Parameters.AddWithValue("@HKid", HKid)
					con1.Open()
...
where HKid=@HKID;"
				...
				con1.Close()
				Session("HKUsername") = TextBoxUsernameReg.Text
				sendMail(ClassSendMail.email, ClassSendMail.pass, TextBoxEmail.Text, TextBoxUsernameReg.Text, "", ClassSendMail.HKViewProfileUrl)
				Response.Redirect("~/HouseKeeper/HKHome.aspx")
...
'		Dim cmd As New SqlClient.SqlCommand()
'		cmd.CommandType = CommandType.Text
'		cmd.CommandText = "select count(UserName) from HouseKeeperTable where UserName=@UserName;"
'		cmd.Connection = con
'		'create a parameterized query
'		cmd.Parameters.AddWithValue("@UserName", userName)
'		'Create DataReader
...
'		Dim cmd As New SqlClient.SqlCommand()
'		cmd.CommandType = CommandType.Text
'		cmd.CommandText = "select count(Email) from HouseKeeperTable where Email=@Email;"
'		cmd.Connection = con
'		'create a parameterized query
'		cmd.Parameters.AddWithValue("@Email", Email)
'		'Create DataReader
...
'				FileUpload1.PostedFile.SaveAs(Server.MapPath("~/ProfilePictures/") + TextBoxUsernameReg.Text + fileName)
'				'Check if username already exists in the DB
'				If (UsernameExistInTable("HouseOwnerTable", TextBoxUsernameReg.Text)) OrElse (UsernameExistInTable("AdminTable", TextBoxUsernameReg.Text)) Then
'					lblStatus.Text = "Username Already Exist, Please Choose Another"
'					lblStatus.ForeColor = System.Drawing.Color.Red
'					TextBoxUsernameReg.Focus()
...
'				ElseIf (EmailExistInTable("HouseOwnerTable", TextBoxEmail.Text)) OrElse (EmailExistInTable("AdminTable", TextBoxEmail.Text)) Then
...
'					cmd1.Parameters.AddWithValue("@UserName", TextBoxUsernameReg.Text)
'					cmd1.Parameters.AddWithValue("@Password", Encrypt(TextBoxConfirmReg.Text))
'					cmd1.Parameters.AddWithValue("@HKId", StudentId)
'					con1.Open()
'					cmd1.ExecuteNonQuery()
'					con1.Close()
'					Session("HKUsername") = TextBoxUsernameReg.Text
...
'				con1.Close()
'				Session("HKUsername") = TextBoxUsernameReg.Text
'				sendMail(ClassSendMail.email, ClassSendMail.pass, TextBoxEmail.Text, TextBoxUsernameReg.Text, "", ClassSendMail.HOViewProfileUrl)
'				Response.Redirect("~/HouseKeeper/HKHome.aspx")
